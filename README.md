...
Projeto desenvolvido para o desafio do 2º Módulo do curso com foco em Back-end da Cubos Academy.

O objetivo (alcançado com sucesso) era criar e desenvolver uma API para um Banco Digital.
(Este é um projeto **piloto**,ou seja, futuramente outras funcionalidades serão implementadas.)
...
**Sempre que a validação de uma requisição falhar, a resposta retornará com código de erro e mensagem adequada à situação.**
...
- Para inicializar esta API será necessário fazer a instalação do **date-fns**(npm i date-fns),**express**(npm i express) e **nodemon**(opcional- npm i -D nodemon)
- A API segue o padrão REST.
- O código está organizado, delimitando as responsabilidades de cada arquivo adequadamente. Ou seja, é esperado que ele tem:
  - Um arquivo index.js
  - Um arquivo de rotas
  - Uma pasta com controladores
- Todos os valores (dinheiro) são representado em centavos (Ex.: R$ 10,00 reais = 1000)

## Persistências dos dados

Os dados serão persistidos em memória, no objeto existente dentro do arquivo `bancodedados.js`. **Todas as transações e contas bancárias são inseridas dentro deste objeto, seguindo a estrutura que já existe.**
...
- este endpoint:
  ...
  - Verificae se o numero da conta passado como parametro na URL é válido
...
- **Resposta**
  ...
  Em caso de **sucesso**, não envia o conteúdo no corpo (body) da resposta.  
  Em caso de **falha na validação**, a resposta possui **_status code_** apropriado, e em seu corpo (body) possui um objeto com uma propriedade **mensagem** que possui como valor um texto explicando o motivo da falha.
...
- este endpoint:
  ...
  - Verificae se o numero da conta e a senha foram informadas (passado como query params na url)
  - Verifica se a conta bancária informada existe
  - Verifica se a senha informada é uma senha válida
